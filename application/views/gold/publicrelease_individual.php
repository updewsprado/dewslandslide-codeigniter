...
<?php
	...
	function parser($internal_alert_level, $suppInfoDesc, $suppInfo, $infoOrComment) 
	{
		...
		if($internal_alert_level == "A1-D" || $internal_alert_level == "ND-D") {
	    	...
	    	$suppInfoDesc = str_replace("[list of date-time (round up to nearest next hour) of succeeding L1-triggering measurements]", date("j F Y, h:i A" , strtotime($list[1])), $suppInfoDesc);
	    	...
	    	$suppInfoDesc = str_replace("[list of date-time (round up to nearest next hour) of succeeding L1/L2-triggering measurements]", date("j F Y, h:i A" , strtotime($list[1])), $suppInfoDesc);
		} else {
