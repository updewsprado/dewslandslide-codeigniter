...
class Narrative_automation_model extends CI_Model {
	...
	public function fetchMessagesFromLastRelease($data) {
		$onset_array = [];
		$onset_query = "SELECT public_alert_release.event_id,public_alert_release.data_timestamp,public_alert_event.event_start FROM public_alert_release INNER JOIN public_alert_event ON public_alert_release.event_id = public_alert_event.event_id WHERE public_alert_event.status = 'on-going' AND public_alert_event.site_id='".$data['event_id']."';";
